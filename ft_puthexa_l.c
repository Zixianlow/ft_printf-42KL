...
/*   Updated: 2022/10/31 17:21:19 by lzi-xian         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

t_list	*ft_puthexa_num_zero_flags(t_list *f, int l)
{
	if (f->fnum >= l)
	{
		f->fnum = f->fnum - l + 1;
...
t_list	*ft_puthexa_flags(t_list *f, int l)
{
	...
	if (f->fdot > 0)
		...
		l = l + f->fdot;
	f = ft_puthexa_num_zero_flags(f, l);
...
int	ft_puthexa_l(unsigned int n, int l, int type)
{
	...
	{
		if (n / 16 > 0)
			l = ft_puthexa_l(n / 16, l, type);
		if (type == 1)
			l = ft_putchar_l(h1[n % 16], l);
		else if (type == 2)
			l = ft_putchar_l(h2[n % 16], l);
	}
	return (l);
}
